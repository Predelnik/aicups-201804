...
public:
  using Arg::operator();
  template <class ActualArg,
            std::enable_if_t<
                !std::is_same<std::decay_t<ActualArg>, self>::value, int> = 0>
...
public:
  ...
  template <class ActualArg0, class... ActualArgs,
            std::enable_if_t<
                !std::is_same<std::decay_t<ActualArg0>, self>::value, int> = 0>
  overload_class(const ActualArg0 &arg0, const ActualArgs &... args)
      : Arg0(arg0), parent_t(args...) {}
};

template <typename T> struct wrapper_for { using type = T; };
...
struct func_ptr_wrapper<Ret(Args...)> {
  ...
  auto operator()(ActualArgs &&... args)
      -> decltype(this->m_ptr(std::forward<ActualArgs>(args)...)) {
...
auto overload(Args &&... args)
    -> detail::overload_class<detail::wrapper_for_t<std::decay_t<Args>>...> {
